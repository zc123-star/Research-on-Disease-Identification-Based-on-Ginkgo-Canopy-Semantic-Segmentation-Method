...
def dice_coeff(x: torch.Tensor, target: torch.Tensor, ignore_index: int = -100, epsilon=1e-6):
    ...
    for i in range(batch_size):
        ...
        # Ensure x_i and t_i are not empty
        if x_i.numel() == 0 or t_i.numel() == 0:
            continue
...
def multiclass_dice_coeff(x: torch.Tensor, target: torch.Tensor,  epsilon=1e-6):
    ...
    for channel in range(x.shape[1]):
        dice += dice_coeff(x[:, channel, ...], target[:, channel, ...], epsilon=epsilon)
...
def dice_loss(x: torch.Tensor, target: torch.Tensor, multiclass: bool = False, ignore_index: int = -100,epsilon=1e-6):
    ...
    return 1 - fn(x, target,epsilon=epsilon )
